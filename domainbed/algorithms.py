...
class SagNet_ViT(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SagNet_ViT, self).__init__(input_shape, num_classes, num_domains,
                                     hparams)
        # featurizer network
        n_outputs, self.network_f = return_backbone_network_without_head(hparams["backbone"])
    ...
    def forward_c(self, x):
        # learning content network on randomized style
        return self.network_c(self.randomize(self.predict_transformer(x), "style"))

    def forward_s(self, x):
        # learning style network on randomized content
        return self.network_s(self.randomize(self.predict_transformer(x), "content"))
    ...
    def predict(self, x):
        return self.network_c(self.predict_transformer(x))
...
def return_backbone_network_without_head(network_name):
    if network_name == "DeitSmall":
        print("DeitSmall Network")
        network = torch.hub.load(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), 'pretrained_models', 'DeiT_models'),
